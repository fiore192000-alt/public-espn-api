...
class TestNameNormalisation:
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Spain: Football-Data abbreviates where Understat spells out.
            ...
            # Germany, where the two sources also disagree on transliteration.
    ...
    @pytest.mark.parametrize(
        "league",
        [
            # Every club that shared a pitch in these leagues, from both sources.
            (
                "Alaves",
                "Almeria",
                "Ath Bilbao",
    ...
    def test_no_two_clubs_in_a_league_resolve_to_the_same_name(self, league):
        """A collision would silently hand one club's shots to another."""
        resolved = [_normalise(name) for name in league]
        assert len(set(resolved)) == len(league)
